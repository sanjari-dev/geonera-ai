...
def get_candles_data(client: Client, instrument: str, timeframe: str, start_time: datetime, end_time: datetime) -> list | None:
    ...
    Uses PREWHERE for optimized ClickHouse querying.
    """
    ...
    query = """
            SELECT timestamp, \
                   ...
                   total_ask_volume, \
                   vwap
            FROM geonera.candles
            PREWHERE
                instrument = %(instrument)s AND
                timeframe = %(timeframe)s AND
                timestamp >= %(start_time)s AND
    ...
    try:
        ...
        logging.info(f"from: {params['start_time']} to {params['end_time']} (exclusive)")
